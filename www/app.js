...
// Configuración de Gemini API y Cobros
let GEMINI_API_KEY = localStorage.getItem("BRAD_GEMINI_API_KEY") || "";
const API_URL = "https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent";

// ============================================================================
// CONFIGURACIÓN DE PAGOS (ADMINISTRADOR)
// ============================================================================
// Tu correo oficial de PayPal para recibir $2.90 USD (equivalente a 1,500 CRC)
const PAYPAL_EMAIL = "soberanos_cr@hotmail.com";

// Tu número de teléfono para SINPE Móvil (Edítalo por tu número real de 8 dígitos, ej: "88888888")
let SINPE_PHONE = "88888888";
let SINPE_OWNER = "Gustavo Duarte";
...
// Redireccionar al usuario a PayPal para Pago Internacional ($2.90 USD ~ 1,500 CRC)
function handlePayPalPaymentRedirect() {
    playUISound('success');
    
    const successUrl = encodeURIComponent("https://meta-hablar-ingles.vercel.app/?payment=success");
    const paypalUrl = `https://www.paypal.com/cgi-bin/webscr?cmd=_xclick&business=${encodeURIComponent(PAYPAL_EMAIL)}&currency_code=USD&amount=2.90&item_name=BRAD%20IA%20Premium%20Subscription&return=${successUrl}`;
    
    alert("[Pasarela PayPal] Te redirigiremos de inmediato a PayPal para procesar tu pago de $2.90 USD con tu tarjeta de crédito, débito o cuenta de PayPal. Al finalizar el pago, presiona 'Volver al comercio' para activar tu Premium al instante.");
    
    window.open(paypalUrl, "_blank");
}

// Configurar los detalles de SINPE en la interfaz
function setupSinpeUI() {
    const phoneEl = document.getElementById("sinpe-phone-display");
    const ownerEl = document.getElementById("sinpe-owner-display");
    const waBtn = document.getElementById("sinpe-whatsapp-btn");
    
    if (phoneEl) phoneEl.innerText = SINPE_PHONE;
    if (ownerEl) ownerEl.innerText = SINPE_OWNER;
    
    if (waBtn) {
        const text = encodeURIComponent(`Hola Gustavo, acabo de realizar el SINPE de 1,500 CRC para activar mi plan Premium de BRAD IA. Adjunto el comprobante.`);
        waBtn.href = `https://wa.me/506${SINPE_PHONE}?text=${text}`;
    }
}

// Alternar la visualización del panel SINPE Móvil
function toggleSinpeDetails() {
    playUISound('click');
    const details = document.getElementById("sinpe-details");
    if (details) {
...
window.addEventListener("load", () => {
    ...
    // DETECTOR DE RETORNO DE PAGO EXITOSO DESDE STRIPE O PAYPAL
